// Fichier : public/assets/js/logic/cloture-logic.js (Version Wizard Corrigée)
...
function switchToCaisseTab(caisseId) {
    const tabSelector = document.querySelector('.tab-selector');
    const targetTab = tabSelector ? tabSelector.querySelector(`[data-tab="caisse${caisseId}"]`) : null;
    if (targetTab) {
        targetTab.click();
    }
}
...
function renderSuggestionTable(suggestions, total) {
    ...
    const rows = suggestions.map(s => {
        ...
        return `<tr><td>${label}</td><td style="text-align: right;">${s.qty}</td><td style="text-align: right;">${formatCurrency(s.total)}</td></tr>`;
    }).join('');
    return `<h4 class="modal-table-title" style="margin-top: 15px;">Suggestion de retrait</h4><div class="table-responsive"><table class="modal-details-table"><thead><tr><th>Dénomination</th><th style="text-align: right;">Quantité à retirer</th><th style="text-align: right;">Valeur</th></tr></thead><tbody>${rows}</tbody><tfoot><tr><td colspan="2"><strong>Total à retirer</strong></td><td style="text-align: right;"><strong>${formatCurrency(total)}</strong></td></tr></tfoot></table></div><div id="withdrawal-hidden-inputs"></div>`;
}

/**
 * NOUVEAU : Fonction dédiée pour préparer et afficher la modale de confirmation.
 * @param {string} caisseId L'ID de la caisse à confirmer.
 */
function showConfirmationModal(caisseId) {
    ...
    const confirmationModal = document.getElementById('cloture-confirmation-modal');
    confirmationModal.querySelector('#confirm-final-cloture-btn').dataset.caisseId = caisseId;
    confirmationModal.querySelector('#cancel-cloture-btn').dataset.caisseId = caisseId;
...
function renderModals(container) {
    container.innerHTML = `
        <div id="caisse-selection-modal" class="modal"><div class="modal-content"><div class="modal-header-cloture"><h3><i class="fa-solid fa-store"></i> Gestion de la Clôture</h3></div><div class="modal-body-cloture"></div><div class="modal-actions" style="justify-content: flex-end;"><button class="btn delete-btn modal-close-btn">Fermer</button></div></div></div>
        <div id="cloture-confirmation-modal" class="modal"><div class="modal-content wide"><div class="modal-header"><h3>Confirmer la clôture de : <span id="confirm-caisse-name"></span></h3><span class="modal-close">&times;</span></div><div class="modal-body"><div id="confirm-caisse-summary"></div><div id="withdrawal-suggestion-container"></div><p class="warning-text">Cette action est irréversible et créera un enregistrement final dans l'historique.</p></div><div class="modal-actions"><button id="cancel-cloture-btn" class="btn delete-btn">Annuler</button><button id="confirm-final-cloture-btn" class="btn save-btn">Confirmer la Clôture</button></div></div></div>
        <div id="cloture-generale-modal" class="modal"><div class="modal-content wide"><div class="modal-header"><h3><i class="fa-solid fa-flag-checkered"></i> Clôture Générale</h3><span class="modal-close">&times;</span></div><div class="modal-body"><p>Toutes les caisses ont été confirmées. Voici le récapitulatif final avant de préparer la journée suivante.</p><div class="accordion-container" id="cloture-generale-summary"></div><div id="cloture-generale-withdrawal-summary"></div></div><div class="modal-actions" style="justify-content: flex-end;"><button class="btn delete-btn modal-close-btn">Annuler</button><button id="confirm-cloture-generale-btn" class="btn save-btn">Lancer la Clôture Générale</button></div></div></div>
        <div id="final-confirmation-modal" class="modal"><div class="modal-content"><div class="modal-header-danger"><h3><i class="fa-solid fa-triangle-exclamation"></i> Êtes-vous absolument sûr ?</h3></div><div style="padding: 20px 0; text-align: center;"><p>Cette action va finaliser tous les comptages et réinitialiser les caisses pour demain. Elle ne peut pas être annulée.</p></div><div class="modal-actions"><button class="btn delete-btn modal-close-btn">Annuler</button><button id="confirm-final-cloture-action-btn" class="btn save-btn">Oui, terminer la journée</button></div></div></div>
    `;
}
...
function attachModalEventListeners(modalsContainer) {
    modalsContainer.addEventListener('click', async (e) => {
        ...
        if (e.target.classList.contains('modal') || target?.classList.contains('modal-close-btn') || target?.classList.contains('modal-close')) {
            ...
            document.getElementById('caisse-selection-modal')?.classList.remove('visible');
            switchToCaisseTab(caisseId);
        } 
        else if (target?.classList.contains('force-unlock-btn')) {
            if (confirm(`Êtes-vous sûr de vouloir forcer le déverrouillage de cette caisse ? L'autre utilisateur perdra la main.`)) {
            ...
            // CORRECTION : L'action de ce bouton est maintenant de montrer la modale de confirmation
            showConfirmationModal(target.dataset.caisseId);
...
export function setupGlobalClotureButton() {
    ...
    clotureBtn.addEventListener('click', async () => {
        ...
        const myLockedCaisse = lockedCaisses.find(c => c.locked_by.toString() === resourceId.toString());
        
        // CORRECTION MAJEURE : Logique du Wizard
        if (myLockedCaisse) {
            // Si l'utilisateur a déjà une caisse de verrouillée, on lui montre directement la confirmation.
            showConfirmationModal(myLockedCaisse.caisse_id);
        } else {
