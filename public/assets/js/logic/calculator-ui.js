    ...
    Object.entries(config.nomsCaisses).forEach(([id, nom], index) => {
        ...
        ecartsHtml += `
            <div id="ecart-display-caisse${id}" class="ecart-display ${isActive}">
                ...
                <div id="main-ecart-caisse${id}" class="main-ecart-display"><span class="ecart-label">Écart Espèces</span><span class="ecart-value">0,00 €</span><p class="ecart-explanation"></p></div>
...
function renderChequeList(caisseId, cheques = [], config) {
    ...
    const chequesHtml = cheques.map((cheque, index) => {
        ...
        return `
            <tr>
                <td>${formatCurrency(parseLocaleFloat(cheque.montant), config)}</td>
        ...
        <div class="cheque-grid">
            <div class="cheque-form-container">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="cheque-amount-${caisseId}" placeholder="0,00 ${config.currencySymbol}">
                </div>
                <div class="form-group">
                    <label for="cheque-comment-${caisseId}">Commentaire</label>
                    <input type="text" id="cheque-comment-${caisseId}" placeholder="Chèque n°12345">
                </div>
            ...
            <div class="cheque-list-container">
                <div class="cheque-list-header">
                    ...
                    <div class="cheque-total" id="cheque-total-container-${caisseId}">
                        Total (${cheques.length} chèque${cheques.length > 1 ? 's' : ''}): <strong>${formatCurrency(totalCheques, config)}</strong>
                    </div>
                </div>
                <div class="cheque-list" id="cheque-list-${caisseId}">
                    ${cheques.length === 0 ? '<p class="empty-list">Aucun chèque ajouté.</p>' : `
...
function renderTpeList(caisseId, terminalId, releves = [], config) {
        ...
        listContainer.innerHTML = sortedReleves.map((releve, index) => {
            ...
            return `
                <div class="tpe-releve-item ${isActive ? 'releve-actif' : ''}">
                    ...
                    <span class="releve-montant">${formatCurrency(parseLocaleFloat(releve.montant), config)}</span>
    ...
    hiddenContainer.innerHTML = sortedReleves.map((r, i) => `
        <input type="hidden" name="caisse[${caisseId}][tpe][${terminalId}][${i}][montant]" value="${r.montant}">
